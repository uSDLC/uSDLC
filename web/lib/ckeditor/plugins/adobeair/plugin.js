...
(function() {
    ...
    function b(c) {
        ...
        for (var g = 0; g < e; g++) {
            f = d.getItem(g);
            (function(h) {
                for (var i = 0; i < a.length; i++)(function(j) {
    ...
    CKEDITOR.plugins.add('adobeair', {init:function(c) {
                if (!CKEDITOR.env.air)return;
                c.addCss('body { padding: 8px }');
                c.on('uiReady', function() {
                    b(c.container);
                    if (c.sharedSpaces)for (var d in c.sharedSpaces)b(c.sharedSpaces[d]);
    ...
    CKEDITOR.ui.on('ready', function(c) {
        ...
        } else if (d instanceof CKEDITOR.dialog)b(d._.element);
    });
})();
