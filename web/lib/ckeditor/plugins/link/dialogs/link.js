...
CKEDITOR.dialog.add('link', function(a) {
	var b = CKEDITOR.plugins.link,c = function() {
		...
		F.hide();
		G.setValue('');
		switch (H) {case 'frame':G.setLabel(a.lang.link.targetFrameName);G.getElement().show();break;case 'popup':F.show();G.setLabel(a.lang.link.targetPopupName);G.getElement().show();break;default:G.setValue(H);G.getElement().hide();break;
		}
	},d = function() {
		...
		var G = F && (F.getAttribute('_cke_saved_href') || F.getAttribute('href')) || '',H,I,J,K,L = {};
		...
		if (F) {
			...
			L.adv = {};
			if (!P) {
				var Q = F.getAttribute('_cke_pa_onclick') || F.getAttribute('onclick'),R = Q && Q.match(n);
				if (R) {
					L.target.type = 'popup';
					L.target.name = R[1];
					var S;
		...
		for (var Z = 0; Z < W.count(); Z++) {
			var aa = W.getItem(Z);
			if (aa.getAttribute('_cke_realelement') && aa.getAttribute('_cke_real_element_type') == 'anchor')Y.push(E.restoreRealElement(aa));
		}
	...
	return{title:D.title,minWidth:350,minHeight:230,contents:[
			...
			{type:'vbox',id:'urlOptions',children:[
				{type:'hbox',widths:['25%','75%'],children:[
					...
					{type:'text',id:'url',label:C.url,required:true,onLoad:function() {
						...
						var E = J.getDialog().getContentElement('info', 'protocol'),F = J.getValue(),G = /^(http|https|ftp|news):\/\/(?=.)/gi,H = /^((javascript:)|[#\/\.\?])/gi,I = G.exec(F);
						if (I) {
		...
		{id:'target',label:D.target,title:D.target,elements:[
			{type:'hbox',widths:['50%','50%'],children:[
				{type:'select',id:'linkTargetType',label:C.target,'default':'notSet',style:'width : 100%;',items:[
					...
					if (E.target)this.setValue(E.target.type);
					c.call(this);
				},commit:function(E) {
			...
			{type:'vbox',width:260,align:'center',padding:2,id:'popupFeatures',children:[
				{type:'fieldset',label:D.popupFeatures,children:[
					...
					{type:'hbox',children:[
						{type:'text',widths:['30%','70%'],labelLayout:'horizontal',label:D.popupWidth,id:'width',setup:r,commit:u},
						{type:'text',labelLayout:'horizontal',widths:['55%','45%'],label:D.popupLeft,id:'left',setup:r,commit:u}
					]},
					{type:'hbox',children:[
						{type:'text',labelLayout:'horizontal',widths:['30%','70%'],label:D.popupHeight,id:'height',setup:r,commit:u},
						{type:'text',labelLayout:'horizontal',label:D.popupTop,widths:['55%','45%'],id:'top',setup:r,commit:u}
					]}
				]}
			]}
		]},
		...
		if ((G = b.getSelectedLink(E)) && G.hasAttribute('href'))F.selectElement(G); else if ((G = F.getSelectedElement()) && G.is('img') && G.getAttribute('_cke_real_element_type') && G.getAttribute('_cke_real_element_type') == 'anchor') {
			H.fakeObj = G;
		...
		if (G.target)if (G.target.type == 'popup') {
			...
			E._cke_pa_onclick = U.join('');
			F.push('target');
		} else {
			if (G.target.type != 'notSet' && G.target.name)E.target = G.target.name; else F.push('target');
			F.push('_cke_pa_onclick', 'onclick');
		}
		if (G.adv) {
			var Z = function(aj, ak) {
				var al = G.adv[aj];
				if (al)E[ak] = al; else F.push(ak);
			};
			if (this._.selectedElement)Z('advId', 'id');
			Z('advLangDir', 'dir');
		...
		if (!this._.selectedElement) {
			...
			if (ab.length == 1 && ab[0].collapsed) {
				var ac = new CKEDITOR.dom.text(G.type == 'email' ? G.email.address : E._cke_saved_href, I.document);
				ab[0].insertNode(ac);
			...
			ad.apply(I.document);
			if (G.adv && G.adv.advId) {
				var ae = this.getParentEditor().document.$.getElementsByTagName('a');
			...
			var af = this._.selectedElement,ag = af.getAttribute('_cke_saved_href'),ah = af.getHtml();
			if (CKEDITOR.env.ie && E.name != af.getAttribute('name')) {
				var ai = new CKEDITOR.dom.element('<a name="' + CKEDITOR.tools.htmlEncode(E.name) + '">', I.document);
				aa = I.getSelection();
				af.moveChildren(ai);
				af.copyAttributes(ai, {name:1});
				ai.replace(af);
				af = ai;
				aa.selectElement(af);
			}
			af.setAttributes(E);
			af.removeAttributes(F);
			if (ag == ah || G.type == 'email' && ah.indexOf('@') != -1)af.setHtml(G.type == 'email' ? G.email.address : E._cke_saved_href);
			if (af.getAttribute('name'))af.addClass('cke_anchor'); else af.removeClass('cke_anchor');
			if (this.fakeObj)I.createFakeElement(af, 'cke_anchor', 'anchor').replace(this.fakeObj);
