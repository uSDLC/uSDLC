...
(function() {
	...
	var c = /^([.\d]*)+(em|ex|px|gd|rem|vw|vh|vm|ch|mm|cm|in|pt|pc|deg|rad|ms|s|hz|khz){1}?/i,d = /^(?:\b0[^\s]*\s*){1,4}$/,e = 0,f;
		...
		if ((j = h.removeAnyChildWithName('cke:listbullet')) && j.length && (j = j[0])) {
			...
			if (i.style)i.style = CKEDITOR.plugins.pastefromword.filters.stylesFilter([
				...
				[/^margin(:?-left)?$/,null,function(m) {
					var n = m.split(' ');
					m = CKEDITOR.plugins.pastefromword.utils.convertToPx(n[3] || n[1] || n[0]);
					m = parseInt(m, 10);
					if (!e && f && m > f)e = m - f;
					i['cke:margin'] = f = m;
				}]
