...
(function() {
	...
	CKEDITOR.dialog.add('usdlc', function(b) {
		function c(k, l) {
			k.setHtml('');
		...
		function f(k) {
			...
			if (m || g.contains(l)) {
				...
				if (o) {
					if (m)p = o.getItem(0); else switch (n) {case 40:p = l.getNext();break;case 38:p = l.getPrevious();break;case 13:case 32:l.fire('click');
					}
					if (p) {
						p.focus();
		...
		;
		CKEDITOR.skins.load(b, 'usdlc');
		var g,h = 'cke_tpl_list_label_' + CKEDITOR.tools.getNextNumber(),i = b.lang.templates,j = b.config;
		return{title:b.lang.templates.title,minWidth:CKEDITOR.env.ie ? 440 : 400,minHeight:340,contents:[
			{id:'selectTpl',label:i.title,elements:[
				{type:'vbox',padding:5,children:[
					{type:'html',html:'<span>' + i.selectPromptMsg + '</span>'},
					{id:'templatesList',type:'html',focus:true,html:'<div class="cke_tpl_list" tabIndex="-1" role="listbox" aria-labelledby="' + h + '">' + '<div class="cke_tpl_loading"><span></span></div>' + '</div>' + '<span class="cke_voice_label" id="' + h + '">' + i.options + '</span>'},
					{id:'chkInsertOpt',type:'checkbox',label:i.insertOption,'default':j.templates_replaceContent}
				]}
			]}
