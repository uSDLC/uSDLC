    ...
    function c(d, e) {
        ...
        function i() {
            this.foreach(function(p) {
                if (/^(?!vbox|hbox)/.test(p.type)) {
                    if (!p.setup)p.setup = function(q) {
                        p.setValue(q.getAttribute(p.id) || '');
                    };
                    if (!p.commit)p.commit = function(q) {
                        var r = this.getValue();
                        if ('dir' == p.id && q.getComputedStyle('direction') == r)return;
                        if (r)q.setAttribute(p.id, r); else q.removeAttribute(p.id);
                    };
                }
            });
        }
        ...
        function j(p) {
            ...
            for (x = 0; x < v.length; x++) {
                ...
                while (t = y.getNextParagraph()) {
                    if (t.getName() in f) {
                        ...
                        for (A = 0; A < B.count(); A++)a(s, B.getItem(A), r);
                    } else {
                        while (!g[t.getName()] && t.getName() != 'body')t = t.getParent();
                        a(s, t, r);
                    }
                }
            }
            ...
            for (x = 0; x < C.length; x++) {
                var G = C[x][0];
                D = G.getParent();
                for (A = 1; A < C[x].length; A++)D = D.getCommonAncestor(C[x][A]);
                F = new CKEDITOR.dom.element('div', p.document);
                for (A = 0; A < C[x].length; A++) {
                    G = C[x][A];
                    while (!G.getParent().equals(D))G = G.getParent();
                    C[x][A] = G;
                }
                var H = null;
                for (A = 0; A < C[x].length; A++) {
        ...
        return{title:d.lang.div.title,minWidth:400,minHeight:165,contents:[
            {id:'info',label:d.lang.common.generalTab,title:d.lang.common.generalTab,elements:[
                {type:'hbox',widths:['50%','50%'],children:[
                    {id:'elementStyle',type:'select',style:'width: 100%;',label:d.lang.div.styleSelectLabel,'default':'',items:[
                        [d.lang.common.notSet,'']
                    ],onChange:function() {
                        m.call(this, ['info:class','advanced:dir','advanced:style']);
                    },setup:function(p) {
                        for (var q in n)n[q].checkElementRemovable(p, true) && this.setValue(q);
                    },commit:function(p) {
                        var q;
                        if (q = this.getValue()) {
            ...
            {id:'advanced',label:d.lang.common.advancedTab,title:d.lang.common.advancedTab,elements:[
                {type:'vbox',padding:1,children:[
                    ...
                    {type:'hbox',children:[
                        {type:'text',id:'style',style:'width: 100%;',label:d.lang.common.cssStyle,'default':'',commit:function(p) {
            ...
            if (e == 'editdiv')o = [this._element]; else o = j(d, true);
            var p = o.length;
            for (var q = 0; q < p; q++) {
            ...
            if (e == 'editdiv')this._element.removeCustomData('elementStyle');
