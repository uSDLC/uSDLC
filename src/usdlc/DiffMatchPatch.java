...
public class DiffMatchPatch {
	 ...
	 */
	protected static LinesToCharsResult diff_linesToChars(String text1,
			String text2) {
	 ...
	 */
	private static String diff_linesToCharsMunge(String text,
			List<String> lineArray, Map<String, Integer> lineHash) {
		int lineStart = 0;
		int lineEnd = -1;
		String line;
	 ...
	 *            List of unique strings.
	 */
	protected static void diff_charsToLines(LinkedList<Diff> diffs,
			List<String> lineArray) {
		StringBuilder text;
		for (Diff diff : diffs) {
	 ...
	 */
	private static String[] diff_halfMatchI(String longtext, String shorttext,
			int i) {
	...
	public void diff_cleanupSemantic(LinkedList<Diff> diffs) {
		...
		while (thisDiff != null) {
			if (prevDiff.operation == Operation.DELETE
				...
				int overlap_length = DiffMatchPatch.diff_commonOverlap(
						deletion, insertion);
				if (overlap_length != 0) {
	 ...
	 */
	public static int diff_xIndex(LinkedList<Diff> diffs, int loc) {
		int chars1 = 0;
		int chars2 = 0;
		int last_chars1 = 0;
		int last_chars2 = 0;
	 ...
	 */
	public static String diff_prettyHtml(LinkedList<Diff> diffs) {
	 ...
	 * @return Source text.
	 */
	public static String diff_text1(LinkedList<Diff> diffs) {
	 ...
	 */
	public static String diff_text2(LinkedList<Diff> diffs) {
		StringBuilder text = new StringBuilder();
		for (Diff aDiff : diffs) {
			if (aDiff.operation != Operation.DELETE) {
				text.append(aDiff.text);
	 ...
	 */
	public static int diff_levenshtein(LinkedList<Diff> diffs) {
		int levenshtein = 0;
		int insertions = 0;
		int deletions = 0;
		for (Diff aDiff : diffs) {
	 ...
	 * @return Delta text.
	 */
	public static String diff_toDelta(LinkedList<Diff> diffs) {
		StringBuilder text = new StringBuilder();
		for (Diff aDiff : diffs) {
			switch (aDiff.operation) {
			case INSERT:
	 ...
	 *             If invalid input.
	 */
	public static LinkedList<Diff> diff_fromDelta(String text1, String delta)
			throws IllegalArgumentException {
		LinkedList<Diff> diffs = new LinkedList<Diff>();
	 ...
	 */
	public static LinkedList<Patch> patch_deepCopy(LinkedList<Patch> patches) {
	...
	public static class Patch {
		...
		/**
		 * Emmulate GNU diff's format. Header1: @@ -382,8 +481,9 @@ Indicies are
